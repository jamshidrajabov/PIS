                ...
                <ul class="dropdown-menu">
                  ...
                  <li class="user-body">
                    <div class="col-xs-6 text-center">
                      <a href="{{route('colleagues.index',['user_id'=>auth()->user()->id])}}">Hamkasblarim</a>
                    </div>  
                    <div class="col-xs-4 text-center">
                      <a href="{{route('mypatients',['user_id'=>auth()->user()->id])}}">Bemorlarim</a>
                    </div>
                  </li>
                  <!-- Menu Footer-->
          ...
          <ul class="sidebar-menu">
            ...
            <li>
              <a href="{{route('mypatients',['user_id'=>auth()->user()->id])}}">
                <i class="fa fa-medkit"></i> <span>Jami bemorlarim</span> 
              </a>
            </li>
            <li>
              <a href="{{route('mycurrentpatients',['user_id'=>auth()->user()->id])}}">
                <i class="fa fa-users"></i> <span>Joriy bemorlarim</span> 
              </a>
            </li>
            <li>
              <a href="{{route('colleagues.index',['user_id'=>auth()->user()])}}">
                <i class="ion ion-person-add"></i> <span>Hamkasblarim</span> 
              </a>
            </li>
            <li>
              <a href="{{route('drugs.index')}}">
                <i class="fa fa-th"></i> <span>Dorilar</span> 
              </a>
            </li> 
            <li>
              <a href="{{route('chat')}}">
                <i class="fa fa-th"></i> <span>Maslahat olish</span>
              </a>
            </li>
            <li>
              <a href="#">
                <i class="fa fa-th"></i> <span>Adminga murojaat</span> <small class="label pull-right bg-green">new</small>
              </a>
            </li>
          ...
          <div class="row">
            <div class="col-lg-3 col-xs-6">
              ...
              <div style="padding-top:20px" class="small-box bg-aqua">
                <div class="inner">
                  <?php 
                    $distinctPatientCount = auth()->user()->periods()->distinct('patient_id')->count('patient_id');
                    ?>
                  <h3>{{$distinctPatientCount}}</h3>
                  <p>Mening jami bemorlarim</p>
                </div>
                <div style="margin: 15px" class="icon">
                  <i class="ion ion-medkit"></i>
                </div>
                <a href="{{route('mypatients',['user_id'=>auth()->user()->id])}}" class="small-box-footer">Ko'proq... <i class="fa fa-arrow-circle-right"></i></a>
              </div>
            </div><!-- ./col -->
            <div  class="col-lg-3 col-xs-6">
              <!-- small box -->
              <div style="padding-top:20px" class="small-box bg-green">
                <div class="inner">
                  ...
                  $currentPatientsCount = \App\Models\Patient::whereIn('id', $periods->pluck('patient_id'))->count();
                  @endphp
                  <h3>{{$currentPatientsCount}}</h3>
                  <p>Mening joriy bemorlarim</p>
                </div>
                <div style="margin:15px" class="icon">
                  <i class="fa fa-users"></i>
                </div>
                <a href="{{route('mycurrentpatients',['user_id'=>auth()->user()->id])}}" class="small-box-footer">Ko'proq...<i class="fa fa-arrow-circle-right"></i></a>
              </div>
            </div><!-- ./col -->
            <div class="col-lg-3 col-xs-6">
              <!-- small box -->
              <div style="padding-top:20px" class="small-box bg-yellow">
                <div class="inner">
                  @php
                    ...
                    // Hospital ID si autentifikatsiya qilingan foydalanuvchi
                    $hospitalId = $user->hospital_id;

                    // Hospital ID si autentifikatsiya qilingan foydalanuvchi bilan birga bo'lgan barcha foydalanuvchilarni hisoblash
                    $usersCount = \App\Models\User::where('hospital_id', $hospitalId)->count()-1;
                    $drugsCount = \App\Models\Drug::all()->count();
                  @endphp
                  <h3>{{$usersCount}}</h3>
                  <p>Mening hamkasblarim</p>
                </div>
                <div style="margin: 15px" class="icon">
                  <i class="ion ion-person-add"></i>
                </div>
                <a href="{{route('colleagues.index',['user_id'=>auth()->user()])}}" class="small-box-footer">Ko'proq...<i class="fa fa-arrow-circle-right"></i></a>
              </div>
            </div><!-- ./col -->
            <div class="col-lg-3 col-xs-6">
              <!-- small box -->
